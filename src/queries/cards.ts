...
/**
 * Money already committed on installment purchases but not yet billed.
 *
 * Grouped BY PURCHASE, not by installment row. A ten-installment purchase with
 * seven already billed produces seven rows in the table; computing "remaining"
 * on each and summing would count the same commitment seven times over. Only the
 * furthest-along installment of each purchase describes what is actually left.
 */
...
export function getOpenInstallments(db: DB = getDb()): OpenInstallment[] {
  const rows = db
    .select({
      accountId: transactions.accountId,
      accountName: accounts.name,
      description: transactions.description,
      installmentNumber: transactions.ccInstallmentNumber,
      totalInstallments: transactions.ccTotalInstallments,
      perInstallmentCents: sql<number>`abs(${transactions.signedCents})`,
      purchaseDate: transactions.ccPurchaseDate,
    ...
    .where(
      and(
        ...
        eq(transactions.isInternal, false),
        eq(accounts.type, 'CREDIT'),
        sql`${transactions.ccTotalInstallments} > 1`,
      ),
    )
    .all();

  // One entry per purchase. purchaseDate identifies it when present; otherwise
  // fall back to description + plan length, which is the best available proxy.
  const byPurchase = new Map<string, (typeof rows)[number]>();
  for (const row of rows) {
    const key = [row.accountId, row.purchaseDate ?? row.description ?? '', row.totalInstallments, row.perInstallmentCents].join('|');
    const seen = byPurchase.get(key);
    if (!seen || (row.installmentNumber ?? 0) > (seen.installmentNumber ?? 0)) {
  ...
  return [...byPurchase.values()]
    .map((row) => {
      ...
      const current = row.installmentNumber ?? 0;
      ...
      return {
        ...
        perInstallmentCents: row.perInstallmentCents,
        remainingCount,
        remainingCents: remainingCount * row.perInstallmentCents,
        lastPostedOn: row.postedOn,
      };
    })
